...
module.exports = function elasticSearchPlugin(schema, options){
  ...
  /* worker for the asynchronous queue*/
  indexWorker = function (model, callback) {
    try {
      ...
      callback({err:error,res:"trap!"});
    }
  }

  /* the asynchronous queue with concurency limit */
  var indexQueue = async.queue(indexWorker, 10);    
  ...
  */
   ...
   * @param maxConnectionNb - maximum document indexion to performe at the same time (default:2000)
   */
  schema.statics.synchronize = function(query, maxConnectionNb){
    var model = this
    ...
    maxConnectionNb = maxConnectionNb || 2000;
    ...
    stream.on('data', function(doc){
      counter++;
      if (counter>maxConnectionNb) 
        stream.pause();

      // what to do when the result of the index known
      var finalize = function(err) {
        try {
          ...
          if (readyToClose && counter === 0)
            close()
          if(counter<maxConnectionNb/10)
            stream.resume();
        } catch(e){/* should never happen but who knows */}
      }

      // try indexing and finalize in any case
      try {indexQueue.push(schema,finalize)} catch(err) {finalize(err)}      
    });
  ...
  /**
   * ElasticSearch delete index function
   *
   * @param callback - callback called when delete is done (err,data)
   */
  schema.statics.deleteIndex = function(cb){
    var model = this;
    setIndexNameIfUnset(model.modelName);
  ...
  function setUpMiddlewareHooks(schema) {
    ...
    schema.post('save', function(){
      ...
      indexQueue.push(this,function(err,res){
        if(err == null)
          model.emit('es-indexed');
        else
          model.emit('es-not-indexed', err,res);
      });
    });
  }
