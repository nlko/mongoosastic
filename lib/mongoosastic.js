...
module.exports = function elasticSearchPlugin(schema, options){
  ...
  indexWorker = function (bulk, callback) {
      bulk = bulk.data   
      esClient.bulk(bulk,function(err){callback(err,bulk.length)});
  }

  /* the asynchronous queue with concurency limit */
  var indexQueue = async.queue(indexWorker, 1);    
  ...
  */
   ...
   */
  schema.statics.synchronize = function(query, batchSize, concurency){
    var model = this
    ...
    indexQueue.concurency = concurency | 1;

    setIndexNameIfUnset(model.modelName);
    var stream = model.find(query).stream();

    // what to do when the result of the batch is known
    var finalize = function(err,length) {
	  counter--;
      try {
        if(err){
          em.emit('error', err);
        }else{
          /* update the number of indexed doc */
          nbDoc+=length;
          em.emit('data', nbDoc);
        }
      ...
      if(bulk.length) {
        ...
        indexQueue.push({data:bulk},finalize);
        bulk=[];
      }
  	}
    ...
    stream.on('data', function(doc){
      // add a new index job 
      bulk.push({index: {index: indexName, type: typeName, id: doc._id.toString(), data: serialize(doc, mapping)}})
